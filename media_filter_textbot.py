...
MONGO_URI = os.getenv("MONGO_URI")  # Koyeb env variable से आएगा
...
@bot.on_message(filters.command("update") & filters.user(filters.user.ADMINISTRATORS))
async def update_db(client, message):
    try:
        user_id = message.from_user.id if message.from_user else None
        member = await client.get_chat_member(CHANNEL_USERNAME, user_id)
        if member.status not in ("administrator", "creator"):
            return await message.reply("Sirf channel admin hi `/update` chala sakta hai.")
